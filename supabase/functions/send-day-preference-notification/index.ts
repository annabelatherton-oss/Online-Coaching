...
// Called by a client's own browser (ClientTraining.jsx) right after they change a day's training
// availability — pushes their coach a heads-up so they see it even before the client's next
// check-in surfaces it in the check-in history. The coach to notify is looked up server-side from
// the CALLER's own client row rather than taken as input, so there's nothing a client request
// could spoof to reach a coach that isn't theirs.
//
// The Web Push encryption helpers below are inlined rather than imported from a shared file on
// purpose: deploying via the Supabase Dashboard's paste-in-one-file editor (the method used for
// every earlier function in this project) has no way to also upload a second, shared file, so a
// cross-file import silently fails there even though it works fine deploying via the CLI. Keeping
// this function fully self-contained means it deploys correctly either way.
...
serve(async (req) => {
  ...
  try {
    const result = await sendPush(subRow.subscription, {
      ...
      url: `/coach/clients/${client.id}?tab=Training`,
    })
