-- Cardio duration and heart rate zone move from being fixed on the library
-- card to being set per assignment (like equipment/variation is for
-- exercises) — a client_schedule_items row now carries its own
-- duration_minutes and heart_rate_zone, chosen when the coach assigns that
-- cardio session to a client's day.
...
ALTER TABLE client_schedule_items
  ...
  ADD COLUMN IF NOT EXISTS heart_rate_zone text;
