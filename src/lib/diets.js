...
// fails Standard if it's marked standard_eligible = false — that's the coach's own deliberate
// call, unlike diet_tags, which get auto-guessed from a meal's ingredients the first time it's
// saved (see MealEditor.jsx's predictMealDietTags call) and so can't be trusted to mean "built
// specifically for this diet" — an ordinary Standard meal like "Chicken & Rice" ends up tagged
// gluten_free/dairy_free purely because chicken and rice happen to contain neither, not because
// it was built as a gluten-free/dairy-free meal. Standard eligibility is checkbox-only; keep any
// meal you don't want offered to a general client "Standard eligible" unchecked. For an actual
// diet, a tag means "this meal is safe for it" — tags are informational there, not exclusive, so
// a tagged meal still shows up in every other diet it also qualifies for. An untagged meal
// (diet_tags empty — e.g. a brand-new meal not yet recomputed) falls back to a live check of its
// ingredients' names.
