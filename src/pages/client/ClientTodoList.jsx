...
const SYSTEM_TASKS = [
  { key: 'macros',   label: 'Hit your macros',           alwaysShow: true },
  { key: 'water',    label: 'Drink 2–3L of water',        alwaysShow: true },
  { key: 'steps',    label: 'Hit 10,000 steps',           alwaysShow: true },
  { key: 'sleep',    label: 'Get 8 hours of sleep',       alwaysShow: true },
  { key: 'training', label: 'Complete your training session', alwaysShow: false },
  { key: 'cardio',   label: 'Complete your cardio',       alwaysShow: false },
]
...
export default function ClientTodoList() {
  ...
  // Notes panel
  const [expandedId, setExpandedId] = useState(null) // 'sys:key' or uuid
  ...
  useEffect(() => {
    supabase.from('clients').select('id').eq('profile_id', session.user.id).single()
  ...
  async function loadForDate() {
    ...
    const [{ data: tasks }, { data: sched }] = await Promise.all([
      ...
      supabase.from('client_schedule_items').select('id, item_type')
  ...
  // --- Derived ---
  const hasTraining = scheduleItems.some(i => i.item_type === 'workout' || i.item_type === 'hiit')
  const hasCardio = scheduleItems.some(i => i.item_type === 'cardio')
  const visibleSystemTasks = SYSTEM_TASKS.filter(t =>
    t.alwaysShow ||
    (t.key === 'training' && hasTraining) ||
    (t.key === 'cardio' && hasCardio)
  )
  const customTasks = dbTasks.filter(t => t.task_type === 'custom')

  const totalTasks = visibleSystemTasks.length + customTasks.length
  const completedCount =
    visibleSystemTasks.filter(t => dbTasks.find(r => r.task_type === 'system' && r.task_key === t.key)?.completed).length +
    ...
    return (
      <div className={`rounded-xl border transition-colors ${
        ...
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          {isPrivate && (
            <span className="text-gray-300 dark:text-gray-600 flex-shrink-0" title="Private">
              <LockIcon />
            </span>
          )}
          {hasNote && !isExpanded && (
            <span className="hidden sm:block text-[10px] text-gray-400 dark:text-gray-600 italic truncate max-w-[120px]">
              {notes}
            </span>
          )}
          <button
            type="button"
            onClick={() => openNotes(uid, notes)}
          ...
          {onDelete && (
            <button
              type="button"
              onClick={onDelete}
              className="p-1 text-gray-300 hover:text-red-400 dark:text-gray-700 dark:hover:text-red-400 flex-shrink-0 transition-colors"
              title="Delete"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              return (
                ...
                >
                  <span className={`text-[9px] uppercase tracking-wide font-semibold ${
                    isSel ? 'text-white/70' : 'text-gray-400 dark:text-gray-500'
                  }`}>
