...
function MealCard({ slotKey, label, optionLabel, cat, mealId, templateMealId, mealMap, mealsByCategory, tier, overrides, onSwap, onViewRecipe }) {
  ...
  return (
    <div className="flex flex-col rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900">
      {/* Photo */}
      <div className="relative aspect-[4/3] bg-gray-100 dark:bg-gray-800 flex-shrink-0">
        {meal?.photo_url ? (
      ...
      <div className="p-3 flex flex-col gap-2 flex-1">
        ...
        <div className="flex items-center gap-2 pt-2 border-t border-gray-100 dark:border-gray-800 mt-auto">
          {meal && (
            <button
              onClick={() => onViewRecipe(slotKey)}
              className="text-xs font-medium text-brand-600 dark:text-brand-400 hover:text-brand-700 flex-1 text-left"
            >
              View recipe →
            </button>
          )}
