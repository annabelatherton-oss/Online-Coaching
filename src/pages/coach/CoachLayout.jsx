...
export default function CoachLayout() {
  ...
  // (navigationType === 'POP') — opening a page fresh, e.g. clicking a sidebar link, should
    ...
    <aside className="flex flex-col w-64 h-full bg-white border-r border-pink-100 text-gray-700">
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        {navItems.map((item, idx) => (
          item.type === 'divider' ? (
            <div key={`divider-${idx}`} className={`${item.label ? 'pt-4 pb-1' : 'pt-2'}`}>
              {item.label && <p className="text-xs font-semibold text-gray-400 dark:text-gray-600 uppercase tracking-wider px-3">{item.label}</p>}
            </div>
          ) : item.disabled ? (
            <div
              key={item.label}
