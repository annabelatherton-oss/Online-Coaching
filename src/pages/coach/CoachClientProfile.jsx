...
function TierIngredientList({ mealId, mealMap, tier, overrides, library, libraryById, dietaryRequirements, onQtyChange, onRemove, onRestore, onAdd, onRemoveAdded, onRevertAll, onToggleStatic, onStaticQtyChange }) {
  ...
  return (
    <div className="space-y-1 pt-1">
      ...
      {/* Meal total and the target/actual comparison are already shown just above, in the card's
          always-visible section — not repeated here to avoid showing the same numbers twice. */}
...
function MealPlanTab({ client, coachId, mealSplit, goalMacroSplits, proteinPerKg, onCalorieTargetChanged }) {
  ...
  // to the old meal's ingredients and wouldn't make sense against a different meal.
  async function changeEverydayMeal(slotKey, mealId) {
    await supabase.from('client_everyday_meals').upsert(
      { client_id: client.id, slot_type: slotKey, meal_id: mealId || null, requested_meal_id: null, needs_coach_review: false, ingredient_overrides: {}, updated_at: new Date().toISOString() },
  ...
  async function load() {
    const [{ data: groups }, { data: asgnData }, { data: past }, { data: mealsData }, { data: libData }, { data: weightRows }, { data: checkinRows }] = await Promise.all([
      ...
      supabase.from('meals').select(`
        ...
        meal_ingredients(id, name, quantity_g, calories, protein_g, carbs_g, fat_g, ingredient_id, is_static, scaling_type, alternative_ingredient_ids),
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        <button onClick={() => navigate(-1)} className="inline-flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors flex-shrink-0">
